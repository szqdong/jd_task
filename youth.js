...
let withdrawUrl = $.getdata('cashurl_zq')||'https://ios.baertt.com/v5/wechat/withdraw2.json';
...
console.log(`============ 脚本执行-北京时间(UTC+8)：${ new Date(UTC8)}  =============\n`)
...
if(!$.isNode()&&cookieYouth.indexOf("#") ==-1){
 cookieArr.push($.getdata('youthheader_zq'));
    ...
    if (process.env.YOUTH_READ && process.env.YOUTH_READ.indexOf('&') > -1) {
      YOUTHREAD = process.env.YOUTH_READ.split('&');
    } else if (process.env.YOUTH_READ && process.env.YOUTH_READ.indexOf('\n') > -1) {
      YOUTHREAD = process.env.YOUTH_READ.split('\n');
    } else {
      YOUTHREAD = [process.env.YOUTH_READ]
    }
...
!(async () => {
  ...
  for (let i = 0; i < cookieArr.length; i++) {
    if (cookieArr[i]) {
      ...
      articlebodyVal = getRandomArrayElements(YOUTHREAD, 1)[0];
...
function CardStatus() {
  return new Promise((resolve, reject) =>{
    $.get(kdHost('WebApi/PunchCard/getMainData?&' + cookie), async(error, resp, data) =>{
      ...
      if (punchcard.code == 1) {
        if (punchcard.data.user.status == 0 && hourFormat(UTC8) > "22") {
          await punchCard()
        } else if (punchcard.data.user.status == 1) {
           $.log("每日打卡已报名，请每天早晨"+cardTime+"点运行打卡");
          detail += `【打卡报名】🔔 待明早${cardTime}点打卡\n`
        } else if (punchcard.data.user.status == 3&&hourFormat(UTC8)==cardTime) {
          $.log("打卡时间已到，去打卡");
          await endCard()
