...
async function doDailyTask() {
  ...
  $.doubleThreeMeal = await request("gotThreeMealForFarm&appid=wh5",{"type":1,"version":2,"channel":1});
  if ($.doubleThreeMeal.code === "0") {
    console.log(`【定时领水】获得${$.doubleThreeMeal.amount}g💧\n`);
    // message += `【定时领水】获得${$.threeMeal.amount}g💧\n`;
  } else {
    // message += `【定时领水】失败,详询日志\n`;
    console.log(`定时领水成功结果:  ${JSON.stringify($.doubleThreeMeal)}`);
  }
  //给好友浇水
...
async function getAwardInviteFriend() {
  ...
  if ($.friendList) {
    ...
    if ($.friendList.friends && $.friendList.friends.length > 0) {
      if($.friendList.countOfFriend>180){				
